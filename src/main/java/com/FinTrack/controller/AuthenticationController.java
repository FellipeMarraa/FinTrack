...
public class AuthenticationController {
    ...
    public ResponseEntity register(@RequestBody @Valid RegisterDTO register){
        ...
        if (usersRepository.findByEmail(register.login()) != null) return ResponseEntity.badRequest().build();

        String encryptedPassword = new BCryptPasswordEncoder().encode(register.password());

        Users newUser = new Users(register.login(), encryptedPassword, register.role());
